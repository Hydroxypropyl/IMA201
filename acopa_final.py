...
def HSI_to_RGB_centers(colors : ArrayType) -> ArrayType :
    """
    Convert the centers from HSI to RGB.
...
def BGR_to_modified_HSI(BGR_image : ArrayType) -> ArrayType:
    """
    Takes the image in BGR and returns the image where each pixel is coded by [H, S, I, 0, 0, 0].
    This coordinate system is used to indicate further on in which segmentation each of the pixel's color coordinates belong.
    ...
    for row_index in range(len(BGR_image)):
        for column_index in range(len(BGR_image[row_index])): 
            pixel = BGR_image[row_index][column_index]
            ...
            HSI_image[row_index][column_index] = [H, S, I, 0, 0, 0]
...
def get_hue_based_segmentation(HSI_image : ArrayType)  -> ArrayType :
    ...
    ---------------
    `image` : 
    the image outputed by BGR_to_modified_HSI
    ...
    #H is better quantified, and is in [-pi, pi]
...
def get_saturation_based_segmentation(hue_segmentation : ArrayType, hue_histogram : ArrayType, HSI_image : ArrayType) -> ArrayType :
    ...
    `hue_histogram` :
    The hue histogram of `HSI_image`

    `HSI_image`:
    The image in modified HSI just as BGR_to_modified_HSI_outputs (each pixel is [H, S, I, i, j, k]) where i,j,k indicate 
    the index of the segmentation to which H, S and I respectively belong to.
    ...
    for line_index in range(len(HSI_image)) : 
        for row_index in range(len(HSI_image[line_index])) :
            ...
            hue_value = pixel[0]
            classified = False
            #go through all the segments to find the one the pixel belongs to, based on hue    
            i = 0
...
def get_intensity_based_segmentation(saturation_segmentation : ArrayType, saturation_histograms : ArrayType, HSI_image ) -> ArrayType :
    """
    Calculates the intensity based segmentation of the different groups of pixesl defined by the segmentation segmentation
    
    Parameters:
    ---------------
    `saturation_segmentation`:
    The saturation segmentation based on the hue segmentation, of `HSI_image`.

    `saturation_histograms` :
    The saturation histograms based on the hue segmentation, of `HSI_image`.
    ...
    `HSI_image` :
    updated image with the j and k components of pixels updated
            ...
            while classified == False and i < len(saturation_segmentation):
                if math.floor(pixel[3]) == i : #the pixel must be in the ith segment of the hue segmentation
                    ...
                    for j in range(len(segment)) :
                        ...
                        else : # start from the end of the previous segment to not have a discontinuity. Could be improved
...
def compute_hue_modes(hue_segmentation : ArrayType, hue_histogram : ArrayType) -> ArrayType : 
    """
    Computes the modes of each of the segments of the hue histogram.
    
    Parameters
    -----------------
    `hue_segmentation`:
    The hue segmentation
    ...
    -----------------
    `hue modes`:
    Array of same shape as `hue segmentation` where each segment contains one value : the mode for the segment
    """
...
def compute_saturation_modes(saturation_segmentation : ArrayType, saturation_histograms : ArrayType) -> ArrayType : 
    """
    Computes the modes of each of the segments of the saturation histogram.
    
    Parameters
    -----------------
    `saturation_segmentation`: array
    The saturation segmentation
    
    `saturation_histogram`:array
    The saturation histogram

    Returns :
    -----------------
    `saturation modes`: array
    Array of same shape as `saturation segmentation` where each segment contains one value : the mode for the segment
    """
...
def compute_intensity_modes(intensity_segmentation : ArrayType, intensity_histograms : ArrayType) -> ArrayType : 
    """
    Computes the modes of each of the segments of the intensity histogram.
...
def acopa(image : ArrayType, HSI : bool = False) -> ArrayType:
    ...
    ---------
    `colors` : array
    The colors in RGB that we are going to use for k-means
    ...
    return HSI_to_RGB_centers(colors), nb_colors
...
def final_palette(img, seeds): 
    ...
    rgb_colors = kmeans.cluster_centers_.round(0).astype(int)
    labels = kmeans.fit_predict(img_array)
    img_quant = np.reshape(rgb_colors[labels],(w,h,d))
    labels = list(labels)
    percent = []
...
fig = plt.figure()
plt.pie(percent,colors=np.array(RGB_colors/255),labels=np.arange(len(RGB_colors)))
plt.show()
